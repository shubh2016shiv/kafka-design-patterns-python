...
This repository uses `uv` with two dependency source-of-truth files:

- `pyproject.toml`:
  Contains declared project dependencies under `[project.dependencies]` and optional
  resilience stack dependencies under `[project.optional-dependencies]`.
- `uv.lock`:
  The lock snapshot generated by `uv` from `pyproject.toml`.
